...
func Request(cmd string, v url.Values) (Response, error) {
	...
	glog.V(1).Infof("request: command: %#v", cmd)
	glog.V(2).Infof("request: url: %s", u)
	resp, err := client.Get(u.String())
	if err != nil {
		return Response{}, util.ErrorLog("query: http request error: %v", err)
	}
...
func createURL(cmd string, v url.Values) (*url.URL, error) {
	...
	if len(base) == 0 {
		return nil, util.ErrorLog("API Endpoint url is empty")
	}
	v.Set("token", config.Cfg.Token)
	u, err := url.Parse(config.Cfg.APIEndpoint)
	if err != nil {
		return nil, util.ErrorLog("Unable to parse API endpoint url: %v", err)
	}
